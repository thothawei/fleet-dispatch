...
### A4. ETA 推播節流 — ✅ 已完成（2026-07-04）
> 客戶 ETA 推播加節流：距上次推播 <N 秒且司機移動 <M 公尺則跳過（預設 30s / 300m，可設）；圍籬抵達通知獨立、不受節流。TrackingService 加 mutex 保護狀態。已 docker 驗證（原地回報 10 次→僅推 2 次；移動 >300m→立即推）。
...
**驗收條件**
- [x] 連續回報下 ETA 推播被節流（實測 10 次回報→2 次推播；預設 30s 間隔下 1 分鐘 ≤3 則）
- [x] 抵達（進入圍籬）仍即時通知，不受節流影響（checkGeofence 獨立於節流）

### A5. 軌跡分區自動維護 — ✅ 已完成（2026-07-04）
> TrackRepository.EnsureTrackPartitions 於啟動時 + 每日排程預建未來 N 個月分區（預設 2），冪等；DropOldTrackPartitions 保留策略（預設 0=關閉）。邊界一律 UTC 午夜對齊 migration。已 docker 驗證（自動建 2026_09、9 月寫入落對分區、10 月無分區如預期報錯）。**修正時區 bug**：GORM 連線 TimeZone=Asia/Taipei 會讓裸日期邊界偏移、與 migration 的 UTC 分區重疊 → 改用顯式 `+00`。
