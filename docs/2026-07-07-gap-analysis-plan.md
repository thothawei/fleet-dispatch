...
- [~] **B2. 地圖叫車**：文字目的地 + 地圖選點接線完成；**待填** Google Maps API key。
- [~] **B3. 即時追蹤**：文字 ETA/距離已通；地圖上看車移動待 key（計畫 Slice 5）。
- [x] **B4. 行程狀態流**：✅ WS + 取消 + 分階段畫面（含 `driver.arrived`）。
- [~] **B5. 完成後評分/付款入口**：✅ 2026-07-08 App 佔位卡已落地（真實 API 待 Phase C；計畫 Slice 6）。
