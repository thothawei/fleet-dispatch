...
- [ ] **O6. 寵物用車清潔費** ✅ **已定案（2026-07-16）：比例加收，上限 30%**
      **規則**：車種為 `pet`（寵物用車）的行程，在車資之上**加收清潔費**；
      ...
      **計算**（`fee_settings.go` `Quote()`）：
      ...
      2. `cleaning_fee = floorNtd(fare × pet_cleaning_fee_bps / 10000)`（**捨去到整數元**，利乘客；
         比照 M 的整數台幣原則，金額必為 100 的倍數）
      3. `total = fare + cleaning_fee`
      **手續費基準待拍板**（見風險 5）：抽成要算在 `fare` 還是 `fare + cleaning_fee` 上？
...
5. ⚠️ **寵物車加收的「觸發條件」未定（最重要）**：
   使用者說法是「車種選單多一種寵物用車，這個車種要加收」，但實務上有兩種解讀：
   - **(a) 依司機車種**：只要司機的車是 `pet`，**所有**搭這台車的行程都加收清潔費
     → 一般乘客搭到寵物車也被多收 30%，**不合理**。
   - **(b) 依乘客需求**：乘客叫車時**指定要寵物用車**（因為要載寵物）→ 派給 `pet` 車種司機 → 加收清潔費。
     **合理，但需要「乘客指定車種」的功能，目前完全沒有**（叫車 API 無車種欄位、
     `dispatch.go` 也不依車種過濾候選司機）。
   **建議 (b)**，但那會多出一整塊工作（叫車帶 `required_vehicle_type`、派單依車種過濾、
   找不到對應車種時的行為）。**需使用者拍板**。
...
- **O. 司機車輛資訊／寵物車清潔費**：車種**選單**（轎車／休旅／七人座／無障礙／寵物用車）＋車牌；
  ...
  **仍待拍板：寵物車加收的觸發條件（依司機車種 vs 乘客指定，見 O 風險 5）、清潔費是否計入抽成、電話隱私。**
