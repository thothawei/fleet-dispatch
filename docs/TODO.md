...
## 💰 M. 金額改用整數台幣（無小數）— 高優先，下次做

> 使用者回報（2026-07-15）：**台幣沒有小數點，支付無法用小數**。目前全系統把金額存「分」
> （`*_cents`，BIGINT）、顯示除以 100，會產生像 `NT$21.03`、`NT$17.96` 這種**不可支付**的金額。
> 車資計算、手續費、會費、遺失物處理費、報表呈現、支付金額**全部**要改成整數台幣。
...
**待決策（先跟使用者確認再動）**：
1. **金額模型**：(A) 維持存「分」但**所有計算/收費/顯示一律進位到整數元**（金額恆為 100 的倍數，schema 不動、
   改運算與顯示）；或 (B) **廢除「分」概念、直接存整數元**（migration 改欄位語意/改名、拔掉所有 ÷100）。
   建議 A（改動面小、不必動歷史資料語意），但需全面確認沒有殘留小數。
2. **進位規則**：四捨五入／無條件進位／捨去——尤其手續費（捨去利司機、進位利總公司）與處理費，屬**商業決策**，要問清楚。
3. **快照一致性**：費率快照制已保護歷史；改規則只影響日後新行程/新帳單，勿回算歷史（沿用既有心智模型）。
...
**驗收方向**：改完後任一端不再出現小數金額；docker E2E 造行程→完成→建協尋單→付款，全程金額為整數元且三端一致；
補/改單元測試斷言 `Quote()`／處理費/報表輸出皆為 100 的倍數。對應前端：
[line-fleet-admin/docs/TODO.md](../../line-fleet-admin/docs/TODO.md)、[line-fleet-app/docs/TODO.md](../../line-fleet-app/docs/TODO.md)。

---

## 下次任務

計費地基 **F1–F8＋F3 里程退路＋F9-1~F9-6 已全數合併進 main**，三端對帳與 F3/F9-3/F9-4 皆 docker E2E 驗過。
**下一個要做的是上面「M. 金額改用整數台幣」（高優先，使用者要求）。** 其餘皆屬「量體上升後才需」的大資料量最佳化，勿過早做：
