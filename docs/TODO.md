...
- [x] **N5. 計費改吃「全程實際路線」** ✅ **已實作（2026-07-17）**（定案 2026-07-16）
      （`internal/osrm/client.go` + `tracking.go` `Complete`）

      **實作內容**：
      1. **`osrm.Client.RouteVia(ctx, []Point)`**：URL 依序串接所有座標（`;` 分隔，`lng,lat` 順序）。
         **既有兩點 `RouteDuration` 完全不動**——派單 ETA 與 F3 既有行為不受影響。
         `Point` 型別自訂而非用 `model.GeoPoint`：osrm 是最底層的外部服務 client，不該依賴領域模型。
      2. **`fallbackVia` 逐段累加**：OSRM 掛掉時逐段 haversine×1.4 再取和。
      3. **`TrackingService.routeDistanceM`**：有未跳過的停靠點（≥2）就走全程多點路線，
         否則退回現行兩點直達（相容單點訂單）。**`billableDistanceM` 取大者的邏輯完全不變**。

      **已跳過的停靠點不計入**（2026-07-17 拍板「司機可跳過」的衍生結論）：
      沒去就沒開那段路，計入等於收乘客沒走的路的錢。軌跡那側本來就自然反映（沒去就沒軌跡）。
      跳過後不足兩點時退回兩點直達，不會失敗。

      **不必把 `ride.PickupPoint` 另外補進路線**：N3 已保證 `rides.pickup_point` ＝ 第一個 pickup stop、
      `dropoff_point` ＝ seq 最大的 dropoff stop，兩者本來就在 stops 裡（補了會重複計算）。

      **驗收**：`go build`／`vet`／`gofmt` 綠。
      單元（`internal/osrm`，無需容器）：URL 依序串接所有座標且為 `lng,lat`、點數不足回 `ErrTooFewPoints`、
      **OSRM 掛掉時逐段累加**（繞路里程 > 直達，且等於手動逐段和）、nil client 走退路。
      整合（真 PostGIS + fake OSRM 伺服器，斷言實際送出的座標）：多停靠點走全程路線、
      **跳過的站不計入**、單點訂單維持兩點直達、全部跳過時退回兩點。
      **反向驗證**：拿掉 `st.Skipped()` 過濾 → 已跳過的座標出現在計費路線，FAIL；
      `fallbackVia` 改成只算頭尾 → 繞路(7040) ＝ 直達(7040)，繞路被漏掉，FAIL。
      **未驗**：docker compose 全服務 live E2E（含真 OSRM）。
...
- [x] **N8. 乘客端行程 API 帶 stops ＋ 進度事件** ✅ **已實作（2026-07-21）**
      ...
      **➡️ 2026-07-28 補：`ride.stop_updated` 也推給司機**（app 端第六輪 debug 抓到）。
      原本只推給乘客，理由寫在程式碼註解裡：「司機端剛做完動作、會自己重讀 active」——
      **那假設了司機只有一台裝置**。同一位司機的第二台裝置（或 LINE 那條路徑）標記時，
      這台不會重讀，畫面停在舊的「下一站」，而「下一站」正是司機端唯一給操作的那一站，
      兩台會對不同的站給按鈕。動作來源那台會多收到一則自己造成的事件（多一次重讀），
      比兩台狀態不一致便宜得多。
      **模擬器實跑對照**：舊版標記後 8 分鐘司機 App 完全沒動；新版 5 秒內
      整份清單跟上（前兩站變綠勾「已完成」、下一站前移、地圖重畫）。
...
## 💰 R. 建單前車資預估報價 API（2026-07-23）

> App 端「懸而未決 #1」拍板：投資一支報價 API，讓乘客在**建單前**看到預估車資，
> 不必到行程結束才知道多少錢——多停靠點行程（N5「車資＝全程實際路線含繞路」）放大了
> 「先搭後知價」的痛點，這是本題主因。**是預估不是定價**：計費仍在完成時依實際行駛路線定格。

- [x] **`POST /api/customer/rides/estimate`（customer JWT，純唯讀）** ✅ 2026-07-23
      新 `service.EstimateService{osrm, fees}`：以全程規劃路線（起點 → 各停靠點 → 終點，**含繞路**）
      算里程，**與完成計費共用同一份 `FeeSettings.Quote`**——預估與實收落在同一套費率規則，
      差異只來自「規劃路線 vs 實際行駛路線」（繞路、跳過站、GPS 軌跡）。
      - 距離走既有 `osrm.RouteVia`（與 N5 計費同一支多點路線 API）；OSRM 掛掉時內部退回逐段
        haversine，故**預估永遠算得出一個近似值**，不因路網引擎暫時不可用就讓乘客卡住。
      - 輸入形狀與建單相同，**沿用** `validateStops`／`sortStopsBySeq`／座標驗證——不另造一套
        驗證邏輯。**目的地座標必填**（沒終點無法路由）；車種選填（寵物車含清潔費）。
      - **多停靠點：全部停靠點都入路線**（預估階段還沒有人被跳過），與計費的 `billableStopPoints`
        （排除已跳過者）刻意不同：那是實際計費，這是全程預估。
      - 回傳**白名單欄位**：`fare_cents`／`cleaning_fee_cents`／`total_cents`／`distance_m`／
        `duration_sec`——**不回** `commission`／`driver_net` 等內部費率（比照 `FeeSettings.CustomerJSON`）。
      - 錯誤對應：座標／車種／停靠點填錯 → 400（沿用建單 `createStatusForErr`）；
        服務未就緒（未注入 osrm/fees）→ 503。
      - 驗收：`internal/service/estimate_test.go` 8 案（單點／多停靠點全程入路線／寵物清潔費／
        缺目的地 400／非法車種／min_fare 下限／未成對停靠點／未就緒）＋
        `internal/handler/estimate_test.go` 2 案（授權 401/403、綁定 400、未就緒 503）。
      - App 端對應：叫車表單顯示預估卡（fleet-app 同批），失敗靜默不擋叫車。
      - **✅ live E2E 對帳（2026-07-23，本機起服務 ＋ m6_pixel）**：預估與完成共用
        `FeeSettings.Quote`＋同一 OSRM 路線 → 不繞路時**實收＝預估**。實測：
        單點 6811m 預估 fare 22100 ＝ 完成 22100；**寵物車**（清潔費率 20%）預估
        fare 22100＋清潔費 4400 ＝ 完成 22100＋4400（車資與清潔費皆一致）；
...
**✅ 模擬器實跑 E2E（2026-07-27，`m6_pixel` ＋ docker compose 三服務，全程截圖）**：
- **migration 000023 真的套用**：容器啟動後 `\d ride_ratings` 顯示表、唯一索引
  `uq_ride_ratings_ride_id`、兩條 CHECK、FK 全部到位。
- **全鏈路**：App 叫車 → 司機 API 接單→上車→完成（ride #26，車資 8500 分）→
  **完成卡「留下評分」是可按的實心按鈕**（不再是 disabled 佔位）→ 4 星＋評論送出 →
  **DB 交叉驗證** `ride_ratings` 寫入 `(26, customer 8, driver 24, score 4, "GreatDriver-B5")`
  → 完成卡當場翻成 ★★★★☆「已評分」、評分按鈕消失。
- **四道守門實打**（直打 API，App 已不給入口）：重複評分 **409**「此行程已評分過」且
  **原分數未被覆寫**；星等 6 → **400**；已取消的行程 → **409**；他人的訂單 → **403**。
- 🐛 **實跑抓到並修掉**：未完成行程的 409 文案是**遺失物的**（見下方 fix 段）。
  單元測試斷言 `errors.Is` 而非文案，所以測不到——這是 live E2E 的直接產出。
- **平均分**：兩趟分別評 4 星與 5 星 → `GET /driver/me` 回 `rating_avg=4.5, rating_count=2`，
  司機 App 收入頁顯示「服務評價 4.5 ／ 5.0（2 則）」，**三處（DB／API／UI）完全一致**。
- 順帶驗到 P4 取消文案兩態：乘客主動取消 →「行程已取消。」；派單逾時無人接 →
  「抱歉，附近暫無可用司機，請稍後再試。」

### 🐛 修正：評分被拒的錯誤訊息講的是遺失物協尋（2026-07-27）

`RateByCustomer` 重用了 `lost_item.go` 的 `ErrRideNotCompleted`，其文案是
「僅已完成的行程可申請遺失物協尋」——而它會**原樣顯示在乘客的評分對話框上**。
兩個功能的**條件**恰好相同（只有已完成且有司機的行程才適用），所以重用 sentinel
看起來很合理，但**面向使用者的文案不是共用的**。

修法：新增 `ErrRideNotRatable`（「僅已完成的行程可以評分」），handler 的 409 對應同步改，
遺失物那條路徑一行不動。測試補「錯誤訊息不得提到遺失物」的斷言，
**反向確認**改回舊 sentinel 會讓 `TestRatingFlow` FAIL。
...
## 🎫 T. App 端實跑回報的後端缺口（2026-07-29 記錄，**只剩 T4 待產品拍板**）

> 來源：fleet-app 的搶單／多裝置／弱網三輪 debug（app PR #62–#66）。
> 這些缺口**都是對真後端逐則印 payload 查證出來的**，但先前只寫在 App 的 TODO 裡，
> 後端這份清單上沒有——所以在這裡補記。App 端已各自做了防線，
> 下面每一項都寫明「後端不改的話，App 只能做到什麼程度」。

**已修**：
- [x] **`ride.stop_updated` 只推給乘客**（PR #53）。`publishStops` 的註解假設「司機剛做完動作、
      會自己重讀 active」——那句話假設司機只有一台裝置。已改為乘客與司機各推一則。
- [x] **`ride.cancelled` 從來沒推給司機**（PR #56，2026-07-29）。`cancelActiveRide` 的
      「通知司機」只有 `line.PushText`——**App 司機一則 WS 事件都收不到**，而司機端沒有輪詢，
      行程卡會一直留著：他繼續開往上車點接一個已經取消的乘客，而後端在取消當下就已把他
      放回 Idle。這是 **PR #52（司機放棄只推 LINE、App 乘客收不到）在司機側的鏡像**。
- [x] **`ride.picked_up` 只推給乘客**（PR #57，2026-07-29）。司機第二台裝置／LINE 標記上車後，
      這台停在「前往上車點」階段，按鈕按下去只會被後端拒絕、「放棄此單」也已不可用。

> **這三個洞是同一次盤點抓到的**：把 13 個事件型別的**實際收件人**列成矩陣
> （逐一看 `s.publish(...)` 的 `Recipient`），再對照 App 兩端的 handler。
> 三者的共同形狀是「**App 端早就寫好處理，後端卻從沒送過**」——那段程式碼一直是死碼。
> 盤點後**剩下的兩個非對稱是刻意的**：`ride.assigned` 只給司機（乘客不需要知道派給誰）、
> `driver.location` 只給乘客與 admin（司機不必收自己的位置）。

**待處理**：

- [x] **T1. 接單／取消失敗回 HTTP 200 ＋ 文案 → 已改成 409**（2026-07-30 修）。
      `AcceptRide` 對「已被別人接走」「非待命狀態」回的是 200＋`{"message":"手慢了…"}`，
      放棄不成時同樣。**後果**：App 若只看有沒有丟例外，沒搶到的司機會拿到一張
      完整但**假的**行程卡（他會開去接一個不存在的乘客）。
      App 端現行防線是「接單後一律重讀 `rides/active`、以後端有沒有這張單判定成敗」——
      **多打一支 API 才換得到正確性**，且逾時時仍要再對帳一次。
      改成 409 之後 App 那條退路可以留著（無害），但錯誤處理會單純很多。
      **改法**：新增四個 sentinel error（`ErrRideTaken`／`ErrDriverNotIdle`／
      `ErrRideStarted`／`ErrRideStateChanged`），service 不再回「(訊息, nil)」；
      `readStatusForErr` 補上這四個 → 409（先前落到 default 會變 500，
      而這些文案是要**原樣顯示給使用者**的）。**文案一字未改**——LINE 與 App 都直接顯示它。
      LINE webhook 的客戶取消也改成回原文案，不再一律「請稍後再試」
      （那會讓人一直重試一件永遠不會成功的事）。
      **驗收**：新增 `dispatch_conflict_test.go` 三案（已被接走／非待命／狀態已變更）＋
      更新既有「已上車不可取消」那案；`go test ./internal/handler ./internal/notify` 綠。
      **實跑驗證**（本機 server，同 U4 的跑法）：重複接單 → `409 {"error":"手慢了，這單已被其他司機接走"}`、
      已上車後乘客取消 → `409 {"error":"行程已開始，無法取消"}`；改之前兩者都是 **200**。
- [x] **T2. 沒搶到的司機收不到任何事件**（PR #59，2026-07-29）。一輪 `dispatchRound` 會把
      同一張單同時推給半徑內每一位待命司機，但 `ride.accepted` **只送給接到的那位**——
      其他人的全螢幕接單卡沒有任何東西收得掉，他得自己按下去拿到「手慢了」才會消失，
      期間卡片蓋著整個畫面。**每一趟車都會發生在所有沒搶到的司機身上。**
      **根因**：`dispatchRound` 的 `offered` 只是遞迴中的記憶體 map，接單／取消流程拿不到。
      **改法**：Redis 加 `ride:<id>:offered`（比照既有的 `rejected`，同 30 分鐘 TTL），
      `pushOffer` 時記錄；接單時對「除了接單者以外」的人送新事件 **`ride.taken`**，送完清集合。
      **不能用 `ride.cancelled` 代替**——訂單沒取消，只是不是他的了。
- [x] **T3. 逾時取消（`giveUpIfUnaccepted`）只通知乘客**（PR #59，同批）。
      改為對所有收過 offer 的司機送 `ride.cancelled`（App 端既有處理就能收掉卡片）；
      **乘客／客服在還沒人接單時取消**（`status=Assigned`）先前同樣沒人通知，一併補上。
- [ ] **T4.（產品決策）多裝置同帳號**。Hub 依 (角色, id) 扇出，**每一條連線都收得到事件**，
      沒有踢舊 session 的機制（JWT 無狀態）。實務影響有兩個：
      ① 兩台同時上線時**位置互相覆蓋**（Redis 單一 `driver:<id>:loc`），派單距離以最後回報的那台為準；
      ② 要不要「一個帳號只能一台上線」屬產品決策，**不該由 App 端片面實作**。
...
- **O. 司機車輛資訊／寵物車清潔費**：車種**選單**（轎車／休旅／七人座／無障礙／寵物用車）＋車牌；
  ...
  **進度：N1–N7 ✅、O1／O2／O3／O4／O6／O7 ✅、P1–P5 ✅
  （O1 為 2026-07-16，其餘皆 2026-07-17）
  → **N、O、P 三章全數完成**，只剩 **O5（admin 呈現／審核，可選、待拍板）**。

  後端能力已到位：司機填車輛才能接單 → 乘客指定車種（可帶多乘客／多停靠點）→
  只派同車種、找不到明確取消 → 司機看得到全程、可標記到站／跳過 →
  完成時依**全程實際路線**（排除已跳過）計費、依**乘客指定車種**加收清潔費、報表分項 →
  乘客看得到車種車牌與司機電話、對話無時間限制。

  **下一項建議：補下游 UI**——後端已跑在兩端前面很遠，做完的能力在 admin 與 App 都還看不到：
  - **admin**：費率頁 `pet_cleaning_fee_bps`、月報表清潔費分項欄、（O5 若要做）車輛審核。
  - **App**（見 [line-fleet-app/docs/TODO.md](../../line-fleet-app/docs/TODO.md)）：
    司機車輛設定頁＋強制跳轉、乘客端車種選擇＋清潔費預告、完成卡分項、
    司機收入頁等式改「營業額 − 手續費 + 清潔費 = 實得」、多停靠點編輯與司機端全程顯示。

  **其餘後端項目**：O5（待拍板）；N 的衍生風險——乘客建單時看不到預估車資
  （繞路越多車資越高），若要「先報價」需要一支以全程路線試算的報價 API，目前**沒有**。**
