...
## 🎫 T. App 端實跑回報的後端缺口（2026-07-29 記錄，**尚未實作**）
...
- [ ] **T2. 沒搶到的司機收不到任何事件**。一輪 `dispatchRound` 會把同一張單同時推給半徑內
      每一位待命司機，但 `ride.accepted` **只送給接到的那位**。其他人的全螢幕接單卡
      沒有任何東西可以把它收掉——只有他自己按下接單、或剛好收到同一 rideId 的
      `ride.cancelled`／`ride.completed` 時才會消失。
      建議：對「本輪收到 offer 但沒接到」的司機補送 `ride.taken`，
      或在 `ride.assigned` 的 payload 帶 `offer_expires_at` 讓 App 自己倒數收卡。
- [ ] **T3. 逾時取消（`giveUpIfUnaccepted`）只通知乘客**，司機端同 T2。
