...
- [ ] **P4. 「找不到指定車種」的行為**（重要）
      指定車種後可用司機**大幅變少**（尤其寵物車／無障礙車），會更常走到
      `giveUpIfUnaccepted` 的自動取消。現行取消訊息是泛用的「附近暫無可用司機」，
      **指定車種時應給更明確的原因**（例如「附近暫無寵物用車」），否則乘客不知道是車種問題還是沒車。
      **待拍板**：要不要提供「找不到指定車種時，改派一般車」的降級選項？
      （寵物車不該降級——載寵物是硬需求；無障礙車更不可降級。**建議不降級**。）

- [ ] **P5. 乘客端可見的加價說明**（與 O6 連動）
      乘客**選擇寵物車的當下**就要知道「會加收清潔費 X%」，不能等到完成才看到。
      後端需提供費率查詢（現行 `GET /api/admin/settings/fees` 是 **admin 專用**，
      乘客端沒有可讀的費率端點）→ **可能要開一支乘客可讀的公開費率 API**（唯讀、只回必要欄位）。

### 風險與待拍板

1. **降級策略**（P4）：找不到指定車種時要不要改派一般車？**建議不降級**（寵物／無障礙是硬需求）。
2. **車種供給**：若車隊裡沒有任何寵物車司機，這個選項在乘客端要不要隱藏／停用？
   需要「有哪些車種目前可用」的查詢（可由 redis 在線司機 + 車種聚合，但會增加派單前的查詢成本）。
3. **乘客可讀的費率 API**（P5）：目前費率只有 admin 能讀。開放給乘客要注意只回「乘客該知道的」
   （清潔費%），**不可外洩手續費 bps／月會費**等內部費率。
...
- **O. 司機車輛資訊／寵物車清潔費**：車種**選單**（轎車／休旅／七人座／無障礙／寵物用車）＋車牌；
  ...
  **仍待拍板：清潔費是否計入抽成／營業額、司機電話隱私、找不到指定車種是否降級改派一般車。**
